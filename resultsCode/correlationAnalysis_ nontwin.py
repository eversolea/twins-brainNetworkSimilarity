...
CSVData = open("nontwins_finalDataMtx.csv")
finalDataMtx = np.loadtxt(CSVData, delimiter=",")
print(np.hsplit(finalDataMtx,2))
nonTwinCorrelation, otherNontwinCorrelation = np.hsplit(finalDataMtx,2)
nonTwinCorrelation = nonTwinCorrelation.flatten()
otherNontwinCorrelation = otherNontwinCorrelation.flatten()

#MZ Twins Correlation vs nontwins Correlation 
plt.hist([nonTwinCorrelation,otherNontwinCorrelation], bins=np.arange(0.75,0.95,0.005), color=['yellowgreen', 'darkred'])
plt.savefig('hist_nonTwins_vs_otherNontwins.png')
plt.clf()
...
print(calculateGroupDifference(nonTwinCorrelation, otherNontwinCorrelation))

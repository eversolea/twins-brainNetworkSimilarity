...
CSVData = open("mz_finalDataMtx.csv")
finalDataMtx = np.loadtxt(CSVData, delimiter=",")
print(np.hsplit(finalDataMtx,4))
mzTwinCorrelation, nontwinCorrelation, ages, sexes = np.hsplit(finalDataMtx,4)
mzTwinCorrelation = mzTwinCorrelation.flatten()
nontwinCorrelation = nontwinCorrelation.flatten()
ages = ages.flatten()
sexes = sexes.flatten()

#MZ Twins Correlation vs nontwins Correlation 
plt.hist([mzTwinCorrelation,nontwinCorrelation], bins=np.arange(0.75,0.95,0.005), color=['yellowgreen', 'darkred'])
plt.savefig('hist_mzTwins_vs_nontwins.png')
plt.clf()

#MZ Twins Correlation vs Age
plt.scatter(mzTwinCorrelation, ages, c ="blue")
plt.savefig('scatter_mzTwins_vs_age.png')
plt.clf()

#MZ Twins Correlation vs Sex
plt.scatter(mzTwinCorrelation, sexes, c ="yellowgreen")
plt.savefig('scatter_mzTwins_vs_sex.png')
...
print(calculateGroupDifference(mzTwinCorrelation, nontwinCorrelation))
